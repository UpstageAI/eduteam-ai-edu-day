...
홈과 Reading List는 상단의 `Upstage Education`·`GitHub` 링크와 자료 목록만 보여 줍니다. 각 항목은 분류, 형식, 제목, 짧은 요약과 이동 링크를 담은 얇은 문서 형태입니다. 데스크톱에서는 약 52px 높이의 문서를 12px씩 겹쳐 쌓고, 이동 링크를 오른쪽에 둡니다. 작은 화면에서는 같은 순서로 줄을 바꾸며 가로 스크롤을 만들지 않습니다.

소개 문구, 중복 메뉴, 푸터, 검색·필터·자료 수·목록의 읽음 표시는 제거했습니다. 실제 글의 글자 크기와 읽음 표시 기능은 유지합니다. 워크숍 안내와 글의 목차도 별도 스크롤 패널 없이 본문 흐름 안에 배치합니다. 링크와 버튼에는 150ms의 색상 전환을 적용합니다. 마우스나 키보드로 선택한 자료는 200ms에 걸쳐 16px 떠오르고, 선택된 동안 다른 문서 위에 머뭅니다. 바깥 영역은 움직이지 않아 목록이 흔들리지 않습니다. 터치 화면에서는 마우스 효과 없이 모든 내용을 읽고 열 수 있으며, 동작 줄이기 설정에서는 이동 효과를 끕니다.
