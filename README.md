...
소개 문구, 중복 메뉴, 푸터, 검색·필터·자료 수·목록의 읽음 표시는 제거했습니다. 실제 글의 제목, 출처, 본문과 원문 그림만 남깁니다. 워크숍 안내와 글은 별도 스크롤 패널 없이 본문 흐름으로 읽습니다. 링크와 버튼에는 150ms의 색상 전환을 적용합니다. 마우스나 키보드로 선택한 자료는 200ms에 걸쳐 16px 떠오르고, 선택된 동안 다른 문서 위에 머뭅니다. 바깥 영역과 실제 링크의 클릭 영역은 움직이지 않아 목록이 흔들리거나 클릭 위치가 바뀌지 않습니다. 터치 화면에서는 마우스 효과 없이 모든 내용을 읽고 열 수 있으며, 동작 줄이기 설정에서는 이동 효과를 끕니다.
