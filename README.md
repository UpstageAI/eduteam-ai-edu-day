...
소개 문구, 중복 메뉴, 푸터, 검색·필터·자료 수·목록의 읽음 표시는 두지 않습니다. 워크숍 안내와 글은 별도 스크롤 패널 없이 본문 흐름으로 읽습니다. 링크와 버튼에는 150ms의 색상 전환을 적용합니다. 마우스로 목록을 훑으면 `assets/drawer.js`가 포인터 위치를 부드럽게 따라가며, 가까운 문서일수록 최대 12px까지 조금 더 올라옵니다. 키보드로 링크를 선택해도 같은 방식으로 움직입니다. 문서의 쌓임 순서와 클릭 영역은 바뀌지 않습니다. 터치 화면, 동작 줄이기 설정, JavaScript를 끈 환경에서는 움직이지 않고 모든 내용과 링크를 그대로 쓸 수 있습니다.
