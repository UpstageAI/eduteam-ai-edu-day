...
Increase actual inter-item overlap to 20px of the 52px sheet, keeping the 16px lift. Move resting text slightly upward into the exposed strip. Wrap each CTA label in a non-interactive span so its visual label can lift while its real 44px anchor stays fixed; apply the same markup to dynamically discovered rows. Lock stationary action geometry, visible-label clicks, bidirectional selection, touch fallback and original source content with regression tests before release.
